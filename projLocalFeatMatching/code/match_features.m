...
function [matches, confidences] = match_features(features1, features2)
  ...
  for i = 1:size(features1, 1)
    % Acha o primeiro e o segundo menor valor
    minValue = e^200;
    minAnterior = e^200;
    minJ = 1;
    
    for j = 1:size(features2, 1)
      % Calcula a diferenca das duas imagens
      value = features1(i, :) - features2(j, :);
      value = sum(value.^2);
      
      % Pego o menor valor e o segundo menor valor
      if(value < minValue)
        minAnterior = minValue;
        minValue = value;
        minJ = j;
      endif
    ...
    # Guardo a posicao do menor valor
    matches(i, 1) = i;
    matches(i, 2) = minJ;
    
    # NNDR = d1 / d2 = ||Da-Db| / ||Da-Dc| formula 4.18
    if(minValue / minAnterior == 0)
      confidences(i) = minValue;
    else
      confidences(i) = 1 / (minValue / minAnterior);
    end
  end

  % Ordena os valores de confidences
