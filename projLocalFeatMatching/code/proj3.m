...
function accuracy = proj3()
  ...
  % YOUR WORK IN HERE!
  [x1, y1, x2, y2, matches, confidences] = compute_correspondences( image1g, image2g, eval_file );
  ...
  show_correspondence(image1, image2, x1(matches(1:num_pts_to_visualize,1)), y1(matches(1:num_pts_to_visualize,1)),  x2(matches(1:num_pts_to_visualize,2)), y2(matches(1:num_pts_to_visualize,2)), 'dots', 'vis_dots.png');
  ...
  [numGoodMatches,numBadMatches,precision,accuracy100] = evaluate_correspondence(image1, image2, eval_file, scale_factor, x1(matches(1:num_pts_to_evaluate,1)), y1(matches(1:num_pts_to_evaluate,1)),  x2(matches(1:num_pts_to_evaluate,2)), y2(matches(1:num_pts_to_evaluate,2)), true ); % Visualize/write result
end